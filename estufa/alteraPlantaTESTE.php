    ...
    <div class="div-cadas-planta">
        <form id="formCadasPlanta" class="form-cadas-planta" method="POST" enctype="multipart/form-data">
            ...
            <div class="campos-planta">
                ...
                <!--Caixa de seleção das estufas que o usuário possui-->
                <div class="half-box spacing">
                    <select name="campoSerie" class="campoSerie" id="campoSerie" required>
                        <option value="">Selecione a estufa</option> <!-- Placeholder -->
                    </select>

                    <script>
                        document.addEventListener('DOMContentLoaded', function() {
                            fetch('selectEstufas.php')
                                .then(response => response.json())
                                .then(data => {
                                    const select = document.getElementById('campoSerie');
                                    select.innerHTML = '<option value="">Selecione a estufa</option>';

                                    if (data.length > 0) {
                                        // Preencher o select com as opções de estufas
                                        data.forEach(planta => {
                                            const option = document.createElement('option');
                                            option.value = planta.id_produto; // Agora usamos o id_produto
                                            option.textContent = planta.n_serie;
                                            select.appendChild(option);
                                        });
                                    } else {
                                        select.innerHTML = '<option value="">Nenhuma estufa encontrada</option>';
                                    }
                                })
                                .catch(error => console.error('Erro ao buscar estufas:', error));
                        });
                    </script>
                </div>
